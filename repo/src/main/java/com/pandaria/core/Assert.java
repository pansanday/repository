...
import com.pandaria.util.IDCardUtil;
...
public abstract class Assert {
    ...
    /** 确保map中key值不为空 */
    public static void isNotBlank(Map<?, ?> map, String key) {
        if (map == null || StringUtils.isBlank(key) || map.get(key) == null || StringUtils.isBlank(map.get(key).toString())) {
            throw new IllegalArgumentException(getMessage(key + "_IS_NULL"));
        }
    }
    ...
    public static void after(Date dateSmall, Date dateBig, String keySmall, String keyBig) {
        notNull(dateSmall, keySmall);
        notNull(dateBig, keyBig);
        if (dateBig.compareTo(dateSmall) <= 0) {
            throw new IllegalArgumentException(getMessage(keyBig + "_NOT_AFTER_" + keySmall));
        }
    }
     ...
     * 手机号
     */
    public static void isPhoneNo(String text) {
        String regex = "((^(13|15|17|18)[0-9]{9}$)|(^0[1,2]{1}\\d{1}-?\\d{8}$)|(^0[3-9] {1}\\d{2}-?\\d{7,8}$)|(^0[1,2]{1}\\d{1}-?\\d{8}-(\\d{1,4})$)|(^0[3-9]{1}\\d{2}-? \\d{7,8}-(\\d{1,4})$))";
        pattern(text, regex, true, getMessage("MOBILE"));
    }

    /**
     * 正则表达式
     */
    private static void pattern(String text, String regex, boolean flag, String key) {
